...
router.get('/api/getDeviceState', function(req, res) {
  res.send( fs.readFileSync('/home/pi/MindLabs/temp.txt', 'UTF-8') );
  console.log("Get Arrived")
});

router.get('/api/getFlowState', function(req, res) {
  res.send( fs.readFileSync('/home/pi/MindLabs/flow.txt', 'UTF-8') );
  console.log("Get Arrived")
});
...
router.post('/api/postTemp', function(req, res) {
  ...
  var stu = req.body;
  var temp = stu.valve;
  fs.writeFile('/home/pi/MindLabs/temp.txt', temp, function(err) {
    if(err) {
      console.log(err);
    }
  })
